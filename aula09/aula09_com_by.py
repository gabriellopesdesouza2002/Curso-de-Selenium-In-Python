...
def espera_elemento(by, webelement, webdriver):
    """
    Qualquer função que espera por um elemento DEVE receber um
    WebDriver e SEMPRE retornar boolean (TRUE OU FALSE)
    
    ### Parâmetros
    * by -> recebida por um partial do functools desse modo -> esperar_button = partial(espera_elemento, By.CSS_SELECTOR)
    * webelement -> recebida por um partial do functools desse modo -> esperar_button = partial(espera_elemento, By.CSS_SELECTOR, 'button')
    
        EXEMPLO:
        esperar_button = partial    (  espera_elemento, By.CSS_SELECTOR,   'button')
        new_function     frunctools    this function    the by parameter   webelement
    ...
    * bool(1) or qualquer número
    ...
    """
    print(f'Tentando encontrar {webelement} By {by}')
    return bool(webdriver.find_elements(by, webelement))
...
esperar_div_sucesso = partial(espera_elemento, By.CSS_SELECTOR, '#finished')
